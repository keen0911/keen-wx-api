...
import com.example.keen.wx.controller.form.*;
import com.example.keen.wx.db.pojo.TbMeeting;
import com.example.keen.wx.exception.KeenException;
import com.example.keen.wx.service.MeetingService;
...
public class MeetingController {
    ...
    public R insertMeeting(@Valid @RequestBody InsertMeetingForm form, @RequestHeader("token") String token){
        if(form.getType()==2&&(form.getPlace()==null||form.getPlace().length()==0)){
            throw new KeenException("线下会议地点不能为空");
        }
        ...
        if(d2.isBeforeOrEquals(d1)){
            throw new KeenException("结束时间必须大于开始时间");
        }
